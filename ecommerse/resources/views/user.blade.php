...
<div class="text-end m-2"><button class="btn btn-primary" data-bs-toggle="modal" data-
bs-target="#ModalTambahUser"> + Tambah user Baru</button></div>
...
<input type="email" name="email" required="required" class="form-
control" value="{{ $p->email }}">
...
<option value="petugas" @if($p->role =='petugas') selected="selected"
@endif>Petugas</option>
<option value="admin" @if($p->role =='admin') selected="selected"
@endif>Admin</option>
</select>
...
<input type="password" name="password" required="required" class="form-
control">
